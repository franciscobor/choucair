...
from sklearn.metrics import mean_squared_error, r2_score  # Análisis de regresión lineal
import numpy as np
...
class Indicadores():
    ...
    def run_dash_indicadores(self):
        ...
        df_CI = pd.read_excel("Z:/Services and Products/Controles/Gestion capacidad/Colaboradores/ultimo Capacidad instalada.xlsx", header=0,
                              sheet_name="Base Información")
        df_CI.loc[:, df_CI.dtypes == 'object'] = df_CI.loc[:, df_CI.dtypes == 'object'].apply(
            lambda row: row.str.upper())

        # Escogemos las columnas de interés
        df_CI = df_CI[["Año", "Mes", "Cliente", "UEN", "Producto", "pais Analista", "Capacida origen", "Area",
                       "Ingresos"]]  # Creamos una columna con un valor numérico para cada mes para poder filtrar los datos por meses
        df_CI = df_CI[(df_CI["Area"] == "OPERACIÓN") & (df_CI["Ingresos"] != 1)]
        df_CI.rename(columns={"pais Analista": "Pais"}, inplace=True)
        df_CI.loc[df_CI["Pais"] == "PERÚ", "Pais"] = "PERU"
        ...
        df_rotacion = pd.read_excel("Z:/Services and Products/Controles/Gestion capacidad/Colaboradores/Rotacion.xlsx", header=0,
                                    sheet_name="Base")
        ...
        df_liberaciones = pd.read_excel("Z:/Services and Products/Controles/Solicitudes/Administración de la Capacidad/Solicitudes Generalistas/Liberaciones v3.2.xlsm",
                                        header=0, sheet_name="Liberaciones")
        df_liberaciones.rename(
            columns={"FechaLiberacion": "Fecha"},
            inplace=True)
        MesCorto = {'ENERO': 'ENE', 'FEBRERO': 'FEB', 'MARZO': 'MAR', 'ABRIL': 'ABR', 'MAYO': 'MAY', 'JUNIO': 'JUN',
                    'JULIO': 'JUL', 'AGOSTO': 'AGO', 'SEPTIEMBRE': 'SEP', 'OCTUBRE': 'OCT', 'NOVIEMBRE': 'NOV',
                    'DICIEMBRE': 'DIC'}
        ...
        df_solicitudes = pd.read_excel("Z:/Services and Products/Controles/Solicitudes/Administración de la Capacidad/Solicitudes Generalistas/Informe Solicitud v9.xlsb",
                                       header=0, sheet_name="Base")

        df_solicitudes["FechaNecesidadSolicitud"].dropna(inplace=True)
        df_solicitudes["FechaNecesidadSolicitud"] = df_solicitudes["FechaNecesidadSolicitud"].apply(lambda row: datetime.fromordinal(datetime(1900, 1, 1).toordinal() + row - 2))
        df_solicitudes["Año"] = pd.DatetimeIndex(df_solicitudes["FechaNecesidadSolicitud"]).year
        df_solicitudes["NumMes"] = pd.DatetimeIndex(df_solicitudes["FechaNecesidadSolicitud"]).month
        ...
        df_solicitudes.loc[:, df_solicitudes.dtypes == 'object'] = df_solicitudes.loc[:,
                                                                   df_solicitudes.dtypes == 'object'].apply(
            lambda row: row.str.upper())
        ...
        def load_disponibles():
            ...
            df_disponibles['Pais'] = df_disponibles['Pais'].apply(lambda row: row.replace('Ú', 'U'))
            df_disponibles['Pais'] = df_disponibles['Pais'].apply(lambda row: row.replace('Á', 'A'))

            return df_disponibles

        df_disponibles = load_disponibles()

        def backup(UEN_drop=None):
            df_disponibles = load_disponibles()
            ...
            df_pendientes["IdentificacionColaborador"] = df_pendientes["IdentificacionColaborador"].astype(str)

            if UEN_drop is not None:
                df_pendientes = df_pendientes[df_pendientes['UEN'] == UEN_drop]
            df_pendientes = df_pendientes.merge(df_panorama, left_on="IdentificacionColaborador",
                                                right_on="Cedula", how="inner").fillna(0)
            ...
            df_solicitudes_firme = df_solicitudes[((df_solicitudes["EstadoSolicitudCO"] == "ABIERTO") |
                                                   (df_solicitudes["EstadoSolicitudCO"] == "EN PROCESO")) & (df_solicitudes['Cliente'].str.contains('CHOUCAIR') == False) &
            ...
            df_buffer = pd.read_excel("Z:/Services and Products/Controles/Solicitudes/Administración de la Capacidad/Solicitudes Generalistas/Buffer.xlsx",
        ...
        def tabla_resumen(years_drop=2023, meses_slider=[1, max_mes], clientes_drop=None, UEN_drop=None,
        ...
        def load_data():
            ...
            # Creamos una columna con un valor numérico para cada mes para poder filtrar los datos por meses
            NumMes = {'ENERO': 1, 'FEBRERO': 2, 'MARZO': 3, 'ABRIL': 4, 'MAYO': 5, 'JUNIO': 6, 'JULIO': 7, 'AGOSTO': 8,
                      'SEPTIEMBRE': 9, 'OCTUBRE': 10, 'NOVIEMBRE': 11, 'DICIEMBRE': 12}
            df_rotacion_tab["NumMes"] = df_rotacion_tab["Mes egreso"].map(NumMes)
            df_rotacion_tab.rename(columns={"Año egreso": "Año", "Mes egreso": "Mes"},
                                   inplace=True)
            return df_rotacion_tab

        df_rotacion_tab = load_data()
        ...
        def load_data_CI():
            ...
            # Escogemos las columnas de interés
            df_CI_tab = df_CI_tab[
                ["Año", "Mes", "Cliente", "UEN", "Producto", "pais Analista", "Capacida origen", "Area",
                 "Ingresos",
                 "Egresos"]]  # Creamos una columna con un valor numérico para cada mes para poder filtrar los datos por meses
        ...
        print(df_CI_tab_raw[(df_CI_tab_raw["Pais"]=="PANAMA") & (df_CI_tab_raw["Año"]==2023)])
        ...
        def tabla_resumen_CI(df_CI, years_drop_CI=2023, meses_slider_CI=[1, max_mes], clientes_drop_CI=None,
                             UEN_drop_CI=None, paises_drop_CI=None, productos_drop_CI=None):
        ...
        df_CI_tab = df_CI_tab_raw[df_CI_tab_raw["Ingresos"] != 1]
        ...
        def load_data_inicial_Solicitudes():
            ...
            df_Solicitudes_tab["País"] = df_Solicitudes_tab["País"].apply(lambda row: row.replace('Ú', 'U'))
            df_Solicitudes_tab["País"] = df_Solicitudes_tab["País"].apply(lambda row: row.replace('Á', 'A'))
        ...
        def data_filtros_Solicitudes(df, years_drop_Solicitudes, meses_slider_Solicitudes, clientes_drop_Solicitudes,
            ...
            if paises_drop_Solicitudes is not None:
                df_chart = df_chart[df_chart["País"] == paises_drop_Solicitudes]
        ...
        def load_data_inicial_Liberaciones():
            ...
            df_liberaciones.rename(
                columns={"FechaLiberacion": "Fecha"},
                inplace=True)
            mes_completo = {"ene": "ENERO", "feb": "FEBRERO", "mar": "MARZO", "abr": "ABRIL", "may": "MAYO",
                            "jun": "JUNIO",
        ...
        def load_data_inicial_Contrataciones():
            ...
            # Escogemos las columnas de interés
            df_Contrataciones_tab["FechaIngresoOP"] = pd.TimedeltaIndex(df_Contrataciones_tab["FechaIngresoOP"] - 2,
                                                                        unit='d') + datetime(1900, 1, 1)
            df_Contrataciones_tab["Año"] = pd.DatetimeIndex(df_Contrataciones_tab["FechaIngresoOP"]).year
            df_Contrataciones_tab["NumMes"] = pd.DatetimeIndex(df_Contrataciones_tab["FechaIngresoOP"]).month
            ...
            df_Contrataciones_tab["País"] = df_Contrataciones_tab["País"].apply(lambda row: row.replace('Ú', 'U'))
            df_Contrataciones_tab["País"] = df_Contrataciones_tab["País"].apply(lambda row: row.replace('Á', 'A'))
        ...
        def load_data_Contrataciones(option="EN FIRME"):
            ...
            if option == "EN FIRME":
                df_Contrataciones_tab = df_Contrataciones_tab[
                    (df_Contrataciones_tab["EstadoSolicitudCO"] == "ABIERTO") |
                    (df_Contrataciones_tab["EstadoSolicitudCO"] == "EN PROCESO")]
            elif option == "FINALIZADAS":
                df_Contrataciones_tab = df_Contrataciones_tab[
                    (df_Contrataciones_tab["EstadoSolicitudCO"] == "FINALIZADO")]
            return df_Contrataciones_tab

        df_Contrataciones_tab = load_data_Contrataciones()
        df_Contrataciones_tab_finalizado = load_data_Contrataciones("FINALIZADO")
        ...
        def data_filtros_Contrataciones(df, years_drop_Contrataciones, meses_slider_Contrataciones,
            ...
            if paises_drop_Contrataciones is not None:
                df_chart = df_chart[df_chart["País"] == paises_drop_Contrataciones]
        ...
        def crecimiento_real(years_drop=2023, clientes_drop=None, UEN_drop=None, paises_drop=None, productos_drop=None,
            ...
            if paises_drop is not None:
                df_Solicitudes = df_Solicitudes[df_Solicitudes["País"] == paises_drop]
            ...
            df_Solicitudes = df_Solicitudes[(df_Solicitudes['UEN']!='CHOUCAIR') &
                                            (df_Solicitudes["EstadoSolicitudCO"] != "CANCELADO") &
                                            (df_Solicitudes["Año"] == years_drop)]
            df_Liberaciones = df_Liberaciones[(df_Liberaciones["EstadoSolicitud"] != "CANCELADO") &
                                              (df_Liberaciones["Año"] == years_drop)]
            df_Rotacion = df_Rotacion[(df_Rotacion['UEN']!='CHOUCAIR') &
        ...
        tab_rotacion = html.Div([
            ...
            dbc.Card([dcc.RangeSlider(id='meses_slider_rotacion', min=1, max=12, value=[1, max_mes], dots=True, step=1,
                                      marks={0: {'style': {'color': 'white'}},
                                             1: {'label': 'Enero', 'style': {'color': 'black', 'font-size': '16px'}},
                                             2: {'label': 'Febrero', 'style': {'color': 'black', 'font-size': '16px'}},
                                             3: {'label': 'Marzo', 'style': {'color': 'black', 'font-size': '16px'}},
        ...
        tab_CI = html.Div([
            ...
            # dbc.Card Crea un cuadro en el que posicionamos elementos. Ayuda visualemente a diferenciarlos
            dbc.Card([dcc.RangeSlider(id='meses_slider_CI', min=1, max=12, value=[1, max_mes], dots=True, step=1,
                                      marks={0: {'style': {'color': 'white'}},
                                             1: {'label': 'Enero', 'style': {'color': 'black', 'font-size': '16px'}},
                                             2: {'label': 'Febrero', 'style': {'color': 'black', 'font-size': '16px'}},
                                             3: {'label': 'Marzo', 'style': {'color': 'black', 'font-size': '16px'}},
            ...
            html.Div([
                html.Div([
                    ...
                    dbc.Card(dbc.CardBody([
                        dcc.Loading(dcc.Graph(id='total-UEN_CI'), color='#119DFF', type='dot', fullscreen=True),
                        dbc.Accordion([dbc.AccordionItem(dash_table.DataTable(
                            id='tabla-UEN_CI',
                            style_header={'font-weight': 'bold', 'color': 'black',
                                          'text-align': 'center',
        ...
        tab_solicitudes = html.Div([
            ...
            # dbc.Card Crea un cuadro en el que posicionamos elementos. Ayuda visualemente a diferenciarlos
            dbc.Card(
                [dcc.RangeSlider(id='meses_slider_Solicitudes', min=1, max=12, value=[1, max_mes], dots=True, step=1,
                                 marks={0: {'style': {'color': 'white'}},
                                        1: {'label': 'Enero', 'style': {'color': 'black', 'font-size': '16px'}},
                                        2: {'label': 'Febrero', 'style': {'color': 'black', 'font-size': '16px'}},
                                        3: {'label': 'Marzo', 'style': {'color': 'black', 'font-size': '16px'}},
        ...
        tab_Liberaciones = html.Div([
            ...
            # dbc.Card Crea un cuadro en el que posicionamos elementos. Ayuda visualemente a diferenciarlos
            dbc.Card(
                [dcc.RangeSlider(id='meses_slider_Liberaciones', min=1, max=12, value=[1, max_mes], dots=True, step=1,
                                 marks={0: {'style': {'color': 'white'}},
                                        1: {'label': 'Enero', 'style': {'color': 'black', 'font-size': '16px'}},
                                        2: {'label': 'Febrero', 'style': {'color': 'black', 'font-size': '16px'}},
                                        3: {'label': 'Marzo', 'style': {'color': 'black', 'font-size': '16px'}},
        ...
        tab_contrataciones = html.Div([
            ...
            # dbc.Card Crea un cuadro en el que posicionamos elementos. Ayuda visualemente a diferenciarlos
            dbc.Card(
                [dcc.RangeSlider(id='meses_slider_Contrataciones', min=1, max=12, value=[1, max_mes], dots=True, step=1,
                                 marks={0: {'style': {'color': 'white'}},
                                        1: {'label': 'Enero', 'style': {'color': 'black', 'font-size': '16px'}},
                                        2: {'label': 'Febrero', 'style': {'color': 'black', 'font-size': '16px'}},
                                        3: {'label': 'Marzo', 'style': {'color': 'black', 'font-size': '16px'}},
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    dbc.Button("PowerBI Dashboard", id="BI_button_Contrataciones", style={'width': '100%'},
                               href='https://plotly.com/', target='_blank'),
                    dcc.RadioItems(['EN FIRME', 'FINALIZADAS'], 'EN FIRME', id="radio_option_Contrataciones")
                ], style={'display': 'flex', 'flex-direction': 'column', 'width': '20%', 'margin-right': '0px'})
            ], style={'display': 'flex', 'margin-top': '0px'})
        ...
        app.layout = dbc.Tabs([
            dbc.Tab(tab_resumen, label="Resumen general"),
        ...
        @app.callback(Output('tabla_buffer', 'data'),
                      ...
                      [Input('paises_drop', 'value')],
                      [Input('productos_drop', 'value')])
        def scatter_chart(UEN_drop, pais, producto):
            """Esta función crea un gráfico de líneas con la rotación del año seleccionado"""
            df = backup(UEN_drop)
        ...
        def scatter_chart(years_drop_rotacion, meses_slider_rotacion, histogram_slider, clientes_drop_rotacion,
            ...
            datos1['Fecha Egreso'] = pd.to_datetime(datos1['Fecha Egreso'])
            datos1['Fecha de ingreso'] = pd.to_datetime(datos1['Fecha de ingreso'])
            datos1['Meses de antigüedad'] = ((datos1['Fecha Egreso'] - datos1['Fecha de ingreso']) / pd.Timedelta(days=30.44)).astype(int)
        ...
        def download_df(years_drop_CI, meses_slider_CI, clientes_drop_CI, UEN_drop_CI, paises_drop_CI,
                        productos_drop_CI, param_drop_CI_download, n_clicks, n):
            """Descarga el datframe seleccionado al hacer click en el botón de descargas"""
        ...
        def update_spikeline(years_drop_CI, meses_slider_CI, clientes_drop_CI, UEN_drop_CI, paises_drop_CI,
            ...
            df_tabla, _ = tabla_resumen_CI(df_CI, years_drop_CI, meses_slider_CI, clientes_drop_CI, UEN_drop_CI,
                                           paises_drop_CI, productos_drop_CI)
        ...
        @app.callback(Output('total-UEN_CI', 'figure'),
                      [Input('years_drop_CI', 'value')],
                      [Input('meses_slider_CI', 'value')],
                      [Input('clientes_drop_CI', 'value')],
                      [Input('UEN_drop_CI', 'value')],
        ...
        def scatter_chart(years_drop_CI, meses_slider_CI, clientes_drop_CI, UEN_drop_CI, paises_drop_CI,
            ...
            return {'data': chart,
                    'layout': {
                        'title': {'text': f'<b>CAPACIDAD INSTALADA POR UEN Y {title}</b>', 'font': {'size': 26}},
                        'xaxis': {'automargin': True},
                        'barmode': 'stack'
                    }}

        @app.callback(Output('tabla-UEN_CI', 'data'),
                      [Input('years_drop_CI', 'value')],
                      [Input('meses_slider_CI', 'value')],
                      [Input('clientes_drop_CI', 'value')],
                      [Input('UEN_drop_CI', 'value')],
        ...
        def update_spikeline(years_drop_Solicitudes, meses_slider_Solicitudes, clientes_drop_Solicitudes,
            ...
            if paises_drop_Solicitudes is not None:
                df_Solicitudes = df_Solicitudes[df_Solicitudes["País"] == paises_drop_Solicitudes]
                title_list.append(paises_drop_Solicitudes)
        ...
        def scatter_chart(years_drop_Solicitudes, meses_slider_Solicitudes, clientes_drop_Solicitudes,
            ...
            df_Solicitudes = df_Solicitudes.groupby(["NumMes", "Mes", "País"])[
                "Cliente"].count().reset_index()
            df_Solicitudes.sort_values(by="NumMes", ascending=True, inplace=True)
            df_Solicitudes.rename(columns={"Cliente": "Solicitudes"}, inplace=True)
            df_Solicitudes = pd.pivot_table(df_Solicitudes, columns="País", values="Solicitudes",
                                            index="Mes", sort=False)
            df_Solicitudes.fillna(0, inplace=True)
        ...
        def scatter_chart(years_drop_Solicitudes, meses_slider_Solicitudes, clientes_drop_Solicitudes,
            ...
            df_Solicitudes_tabla = pd.pivot_table(df_Solicitudes, columns="Mes", values="Solicitudes",
                                                  index="País", sort=False).reset_index()
            df_Solicitudes_tabla.fillna(0, inplace=True)
            df_Solicitudes_tabla["TOTAL"] = df_Solicitudes_tabla.iloc[:, 1:].sum(axis=1)
            df_Solicitudes_tabla.sort_values(by="TOTAL", ascending=False, inplace=True)
        ...
        def scatter_chart(years_drop_Solicitudes, meses_slider_Solicitudes, clientes_drop_Solicitudes,
            ...
            df_Solicitudes = df_Solicitudes.groupby(["País", "UEN", "Producto"])[
                "Cliente"].count().reset_index()
            df_Solicitudes.rename(columns={"Cliente": "Solicitudes"}, inplace=True)
            df_Solicitudes['Porcentaje'] = df_Solicitudes['Solicitudes'].apply(
                lambda row: round(row * 100 / df_Solicitudes["Solicitudes"].sum()))

            chart = px.sunburst(df_Solicitudes, path=["País", "UEN", "Producto"], values="Porcentaje", maxdepth=2)
        ...
        def scatter_chart(clientes_drop_Liberaciones, UEN_drop_Liberaciones,
            ...
            df_pendientes = df_liberaciones[(df_liberaciones["EstadoSolicitud"] == "ABIERTO") |
                                            (df_liberaciones["EstadoSolicitud"] == "EN PROCESO")]
            df_pendientes.loc[df_pendientes["Pais"].isna(), "Pais"]="Sin País"
            ...
            df_disponibles_ = df_disponibles_.groupby(["Pais", "Producto"])["Analista"].count().reset_index()
            df_disponibles_.rename(columns={"Analista": "Disponibles"}, inplace=True)

            df_pendientes = df_pendientes.merge(df_panorama, left_on="IdentificacionColaborador",
                                                right_on="Cedula", how="inner").fillna(0)


            df_pendientes = df_pendientes.groupby(["PAÍS ANALISTA", "Producto"])["IdentificacionColaborador"].count().reset_index()
            df_pendientes.rename(columns={"IdentificacionColaborador": "Pendientes de liberación", "PAÍS ANALISTA": "Pais"}, inplace=True)

            df_pendientes.loc[df_pendientes["Producto"] == "MIGRACIÓN", "Producto"] = "MIGRACION"
            df_disponibles_.loc[df_disponibles_["Producto"] == "MIGRACIÓN", "Producto"] = "MIGRACION"

            df1 = df_disponibles_.merge(df_pendientes, on=["Pais", "Producto"], how="outer").fillna(0)
            df1["Total"] = df1["Disponibles"] + df1["Pendientes de liberación"]
            df1["Producto"] = df1["Producto"].astype(str)
            sum_row = pd.DataFrame(df1.sum()).T
            df1 = pd.concat([df1, sum_row], ignore_index=True)
            df1.iloc[-1, 0] = "TOTAL"
            df1.iloc[-1, 1] = ""
            df1.rename(columns={"Pais": "País analista"}, inplace=True)
            df1.loc[df1["Producto"]=="MIGRACIÓN", "Producto"]="MIGRACION"
        ...
        def download_df(clientes_drop_Liberaciones, UEN_drop_Liberaciones,
            ...
            df_disponibles_ = df_disponibles_[["Analista", "Ultimo Cliente", "UEN", "Motivo Liberacion",
                                              "Fecha de Liberación", "Producto", "Ciudad de residencia"]]

            changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
            if 'button_analistas_disponibles' in changed_id:
                return dcc.send_data_frame(df_disponibles_.to_excel,
        ...
        def update_spikeline(years_drop_Contrataciones, meses_slider_Contrataciones, clientes_drop_Contrataciones,
            ...
            if paises_drop_Contrataciones is not None:
                df_Contrataciones = df_Contrataciones[df_Contrataciones["País"] == paises_drop_Contrataciones]
                title_list.append(paises_drop_Contrataciones)
            ...
            df_Contrataciones = df_Contrataciones.groupby(["NumMes", "Mes", "EstadoSolicitudCO"])[
                "Cliente"].count().reset_index()
            ...
            df_Contrataciones = pd.pivot_table(df_Contrataciones, columns="EstadoSolicitudCO", values="Contrataciones",
                                               index="Mes", sort=False)
        ...
        def scatter_chart(years_drop_Contrataciones, meses_slider_Contrataciones, clientes_drop_Contrataciones,
            ...
            df_Contrataciones = df_Contrataciones.groupby(["NumMes", "Mes", "EstadoSolicitudCO"])[
                "Cliente"].count().reset_index()
            df_Contrataciones.sort_values(by="NumMes", ascending=True, inplace=True)
            df_Contrataciones.rename(columns={"Cliente": "Contrataciones", "EstadoSolicitudCO": "Estado"}, inplace=True)
        ...
        def scatter_chart(years_drop_Contrataciones, meses_slider_Contrataciones, clientes_drop_Contrataciones,
            ...
            df_Contrataciones = df_Contrataciones.groupby(["NumMes", "Mes", "País"])[
                "Cliente"].count().reset_index()
            df_Contrataciones.sort_values(by="NumMes", ascending=True, inplace=True)
            df_Contrataciones.rename(columns={"Cliente": "Contrataciones"}, inplace=True)
            df_Contrataciones = pd.pivot_table(df_Contrataciones, columns="País", values="Contrataciones",
                                               index="Mes", sort=False)
            df_Contrataciones.fillna(0, inplace=True)
        ...
        def scatter_chart(years_drop_Contrataciones, meses_slider_Contrataciones, clientes_drop_Contrataciones,
            ...
            df_Contrataciones_tabla = pd.concat([df_Contrataciones_tabla, sum_row], ignore_index=True)
            df_Contrataciones_tabla.iloc[-1, 0] = "TOTAL"
            df_Contrataciones_tabla["COMPARATIVO"] = df_Contrataciones_tabla.iloc[:, -1] - df_Contrataciones_tabla.iloc[
                                                                                           :, 1]
            return df_Contrataciones_tabla.to_dict('records')
        ...
        def scatter_chart(years_drop_Contrataciones, meses_slider_Contrataciones, clientes_drop_Contrataciones,
            ...
            df_Contrataciones = df_Contrataciones.groupby(["País", "UEN", "Producto"])[
                "Cliente"].count().reset_index()
            df_Contrataciones.rename(columns={"Cliente": "Contrataciones"}, inplace=True)
            ...
            chart = px.sunburst(df_Contrataciones, path=["País", "UEN", "Producto"], values="Porcentaje", maxdepth=2)
